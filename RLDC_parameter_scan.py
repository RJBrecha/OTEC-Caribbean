...
ScaleMax = 201
X = np.arange(1,ScaleMax,2)
Y = np.arange(1,ScaleMax,2)
...
for W in X:
    for S in Y:
        ...
        for i in range(1,1001,10):
            ...
            RL2 = []
            #BatteryMaxPower = min(10,BatteryCap)
            BatteryMaxPower = max(PeakLoad,BatteryCap/BatteryHours)
            Batt = BatteryCap*.75
...
ax.set_ylabel('Solar pv capacity [MW]')
ax.set_zlabel('Battery storage [MWh]')

fig.tight_layout()
plt.show()
fig.savefig(r"C:\Users\Robert Brecha\Documents\Papers\OTEC and SWAC survey\DispCap_10d_new.png")

np.savetxt(r"C:\Users\Robert Brecha\Documents\Papers\OTEC and SWAC survey\DispCap_10d_new.txt",Out)
