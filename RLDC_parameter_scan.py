...
fulldf = pd.read_csv(r"C:\Users\Robert Brecha\Documents\Papers\OTEC and SWAC survey\Residual load data Antigua.csv")
...
ScaleMax = 301
X = np.arange(1,ScaleMax,10)
Y = np.arange(1,ScaleMax,10)
...
for W in X:
    for S in Y:
        ...
        for i in range(1,2001,25):
            BatteryCap = i
            B = []
            RL2 = []
            #BatteryMaxPower = min(10,BatteryCap)
            #BatteryMaxPower = min(PeakLoad,BatteryCap/BatteryHours)
            BatteryMaxPower = PeakLoad
            Batt = BatteryCap*.75
...
#                    B.append(Batt)
            #    print(Batt)
                B.append(Batt)    
                RL2.append(x) 
            if sum(i for i in RL2 if i > 0) <=0.001*sum(LoadData):
#                print(BatteryCap)
...
clb = fig.colorbar(surf, pad=0.1)
clb.ax.set_title('MWh')
#clb.set_ticks([0,1000])


ax.set_ylim(ScaleMax, 0)
#ax.set_zlim(0,4500)
...
ax.set_xlabel('Wind [MW]')
ax.set_ylabel('Solar pv [MW]')
ax.set_zlabel('Battery storage [MWh]')

fig.tight_layout()
plt.show()
fig.savefig(r"C:\Users\Robert Brecha\Documents\Papers\OTEC and SWAC survey\AntigDispCap_20d.png")

np.savetxt(r"C:\Users\Robert Brecha\Documents\Papers\OTEC and SWAC survey\AntigDispCap_20d.txt",Out)
